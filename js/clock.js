...
const btn_hover_list = [clockBtnBack, secondBtnBack, timerBtnBack, alarmBtnBack];
...
class Clock {
    constructor (hour, minute, second) {
        ...
        this.hourAngle = hour * 30 + minute * 0.5 + second * 0.008333333333333333; // 记录时针角度
        if (hour >= 12) {
    ...
    update_angle_via_time () {
        ...
        this.hourAngle = (this.hour % 12) * 30 + this.minute * 0.5 + this.second * 0.008333333333333333;
    }
...
const clock = get_current_clock(); // 创建时钟对象
...
function second_mousemove (event) {
    let angle = (-Math.atan2(125 - event.offsetX, 125 - event.offsetY) * 180) / Math.PI;
    angle.toFixed(2);
...
function minute_mousemove (event) {
    const angle = (-Math.atan2(125 - event.offsetX, 125 - event.offsetY) * 180) / Math.PI;
    angle.toFixed(2);
...
function update_time_text () {
    ...
    minuteText.innerHTML = clock.minute < 10 ? '0' + clock.minute : clock.minute;
    secondText.innerHTML = clock.second < 10 ? '0' + clock.second : clock.second;
}
...
function allowDrop () {
    ...
    minuteHand.onmousedown = function () {
        hourHand.setAttribute('style', 'rotate: ' + clock.hourAngle + 'deg');
        secondHand.setAttribute('style', 'rotate: ' + clock.secondAngle + 'deg');
        liveClock.setAttribute('style', 'rotate: ' + (clock.secondAngle + 180) + 'deg');
